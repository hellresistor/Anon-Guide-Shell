...
mkdir $PWD/storage/gpg-revoke 
gpg --cipher-algo AES256 --symmetric $EasyNameToRevokeCert
mv $PWD/*.gpg $PWD/storage/gpg-revoke
...
mkdir $PWD/.config/hexchat/certs 
cd $PWD/.config/hexchat/certs 
...
read -p "Now BACK TO DEbian HOST and PRESS THE KEY to continue 00-AnonGuide.sh "
