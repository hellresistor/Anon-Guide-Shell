...
YourDeviceName=$(sudo awk '{print $2}' /etc/crypttab) 
...
if [ "$WNXFAC" == "CLI" ]
then
 read -r -p "Will Run Whonix Gateway on CLI...
Press <Enter> Key to continue..."
 sudo virsh start Whonix-Gateway

 read -r -p "Will Run Whonix Workstation on CLI...
Press <Enter> Key to continue..."
...
read -r -p "Change screen Resolution: "
...
read -r -p "### VERIFY WHONIXes VMs ARE ALL SHUTDOWN!!! ###"
}
